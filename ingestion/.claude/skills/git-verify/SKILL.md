...
- `.env` (contains real DB credentials)
- `application.properties` (may contain DB credentials or Notifier service URLs)
...
For each staged file, search for:
- Strings that look like passwords: `password\s*=\s*["']?[^\s${}]` (not a placeholder like `${...}`)
- Literal DB URLs with credentials: `jdbc:postgresql://[^$].*:[^$].*@`
- Hardcoded service URLs that should come from env vars: `http://localhost` or `http://notifier` hardcoded as a string literal (not in `application.properties`)
...
Run `git diff --staged --stat` to show a compact summary: which files changed, how many lines added/removed.
...
- Warn if the branch is `main` or `master` — direct commits to main are not allowed; open a PR instead.
